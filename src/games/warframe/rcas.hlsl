...
float3 ApplyRCAS(float3 b, float3 d, float3 center_color, float3 f, float3 h) {
  if (RENODX_FX_SHARPENNING_STRENGTH == 0.f) return center_color;  // Skip sharpening if amount is zero

#define ENABLE_NOISE_REMOVAL           0u  // Always good to be enabled
#define ENABLE_NORMALIZATION           1u
#define SHARPENING_NORMALIZATION_POINT 100
  ...
  float hitMinLum = min4Lum * rcp(4.f * limited_max4Lum);
  float hitMaxLum = (peakC.x - limited_max4Lum) * rcp(4.f * min4Lum + peakC.y);
...
#define FSR_RCAS_LIMIT 0.1875f
  ...
  float lobe = max(float(-FSR_RCAS_LIMIT), min(localLobe, 0.f)) * RENODX_FX_SHARPENNING_STRENGTH;
