...
float4 main(
  ...
  SV_Target.rgb = renodx::color::srgb::EncodeSafe(float3(SV_Target.x, SV_Target.y, SV_Target.z));
  if (RENODX_TONE_MAP_TYPE != 0.f) {
    SV_Target.rgb = renodx::draw::UpscaleVideoPass(SV_Target.rgb);
  }

  SV_Target.rgb = renodx::draw::RenderIntermediatePass(renodx::color::srgb::DecodeSafe(SV_Target.rgb));
  SV_Target.w = _90;
  return SV_Target;
}
